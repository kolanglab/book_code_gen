...
最適化は奥が深く、それだけで何冊もの本があります [](#cite:muchnick1997)。
...
[](#cite:mckeeman1965)。これは、生成された命令列を小さな窓（のぞき穴）で
...
> [!NOTE]
> `fold` が `:div` を扱っていないのは手抜きではありません。`6 / 0` のような式を
> コンパイル時に畳もうとすると、コンパイラ自身がゼロ除算で落ちるか、本来
> **実行時に**起きるはずだったエラーを握りつぶすことになり、章冒頭の大原則
> 「プログラムの意味を変えてはならない」に抵触します。割り算を畳むなら
> 「除数が 0 でないと分かるときだけ」という条件が要ります。畳み込みひとつにも、
> 意味を保つための配慮が顔を出すのです。
...
ただし、CSE も DCE も無条件には適用できません。まとめたり消したりしてよいのは、
**副作用のない計算**に限られます。関数呼び出し（中で何をするか分からない）、
メモリへの書き込み、入出力、例外を投げうる演算（ゼロ除算など）は、結果が使われて
いなくてもプログラムの観測できる振る舞いに影響するため、見た目が同じでもまとめられず、
未使用でも消せません。定数畳み込みが `:div` を避けたのと同じく、最適化は「対象言語の
意味を変えない範囲で」という大前提のもとで行います。
...
たびに新しい名前を振った中間表現でした。なお、ここで名前を振り直す「変数」は
IR 上の一時変数（仮想レジスタ）のことです。配列の要素やオブジェクトのフィールドの
ようなメモリ上の場所まで SSA の名前になるわけではなく、メモリの読み書きをまたぐ
最適化には別の解析（エイリアス解析など）が必要になります。
...
解析なしに素直に書けるようになります [](#cite:cytron1991)。
...
求める方法を確立したのが[](#cite:cytron1991)の有名な仕事で、
今日の LLVM をはじめ多くのコンパイラ基盤が SSA を中心に据えています
[](#cite:lattner2004)。最終的にコードを生成する前には、
...
段階で、それぞれに向いた最適化を行っていくものです [](#cite:cooper2011)。
