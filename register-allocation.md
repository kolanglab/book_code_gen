...
もっとも大きく左右する処理のひとつです [Cooper and Torczon, 2011](#cite:cooper2011)。
...
情報を伝播させることで求めます [Muchnick, 1997](#cite:muchnick1997)。本章では
...
[Chaitin, 1982](#cite:chaitin1982)。中心となるのは、次の素朴な観察です。
...
# => {"d"=>0, "a"=>0, "b"=>1, "c"=>2}  （順序は実装依存）
```

`a`・`b`・`c` には別々の色 `0`・`1`・`2` が割り当てられ、`d` は `a` と同じ色 `0` を
共有できました。3個のレジスタにきれいに収まったわけです。もしレジスタが2個
...
Chaitin の方式には改良が重ねられてきました。なかでも[Briggs et al., 1994](#cite:briggs1994)の
